...
WPplant::WPplant(const string& symbol, const std::string& location, int turbine_type, int number_WT, const vector<double>& parameters) : RES_base(symbol, location) {
	// Default parameters for a WP plant can be set here if needed
	if (turbine_type == 3) {
		// Initialize WT type 3 parameters
		WTtype3* wt = new WTtype3(symbol, location, parameters);
		Y_matrix = wt->get_y_parameters(); // Example of using WTtype3's Y_matrix
		wind_turbine = wt;
	}
	else if (turbine_type == 4) {
		// Initialize WT type 4 parameters
		WTtype4* wt = new WTtype4(symbol, location, parameters);
