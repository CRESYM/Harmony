...
elapsed_time = @elapsed begin
    ...
    for i in 1:nconvs
        ...
        if conv[i, 4] == 1  # P control
            @constraint(model, ps[i] == -conv[i, 6] / baseMW_dc)
        elseif conv[i, 4] == 2  # V control
            @constraint(model, vn_dc[i] == conv[i, 8])
        else  # Droop control
            @constraint(model, pn_dc[i] == (conv[i, 24] - 1 / conv[i, 23] * (vn_dc[i] - conv[i, 25])) / baseMW_dc * (-1))
        end

        # AC side
        if conv[i, 5] == 1  # Q control
        @constraint(model, qs[i] == -conv[i, 7] / baseMW_dc)
        else  # V control
    ...
    sl2fwd_ac = Vector{Any}(undef, ngrids)
    sl2bwd_ac = Vector{Any}(undef, ngrids)
    theta_diff_ft =  Vector{Any}(undef, ngrids)
    theta_diff_tf = Vector{Any}(undef, ngrids)
    plfwd_ac = Vector{Any}(undef, ngrids)
    qlfwd_ac = Vector{Any}(undef, ngrids)
    plbwd_ac = Vector{Any}(undef, ngrids)
    qlbwd_ac = Vector{Any}(undef, ngrids)
    ...
    for ng = 1:ngrids
        ...
        var_size_ac = 4*nbuses_ac[ng] + 2*ngens_ac[ng]+2*nbranches_ac[ng]
        ...
        # 7. lb and ub of ac branches
        k = 4 * nbuses_ac[ng] + 2*ngens_ac[ng]
        lb_ac[ng][k+1:k+nbranches_ac[ng]] .= 0
        ub_ac[ng][k+1:k+nbranches_ac[ng]] .= Inf

        # 8. lb and ub of ac branches
        k = 4 * nbuses_ac[ng] + 2*ngens_ac[ng] + nbranches_ac[ng]
        lb_ac[ng][k+1:k+nbranches_ac[ng]] .= 0
        ub_ac[ng][k+1:k+nbranches_ac[ng]] .= Inf
    ...
    for ng = 1:ngrids
        var_size_ac = 4*nbuses_ac[ng] + 2*ngens_ac[ng]+2*nbranches_ac[ng]
        var_ac[ng] = @variable(model, [1:var_size_ac])
        ...
        sl2fwd_ac[ng] = var_ac[ng][4*nbuses_ac[ng]+2*ngens_ac[ng]+1:4*nbuses_ac[ng]+2*ngens_ac[ng]+nbranches_ac[ng]]
        sl2bwd_ac[ng] = var_ac[ng][4*nbuses_ac[ng]+2*ngens_ac[ng]+nbranches_ac[ng]+1:4*nbuses_ac[ng]+2*ngens_ac[ng]+2*nbranches_ac[ng]]
        # pij_ac[ng] = @variable(model, [1:nbuses_ac[ng], 1:nbuses_ac[ng]])
        # qij_ac[ng] = @variable(model, [1:nbuses_ac[ng], 1:nbuses_ac[ng]])
        qlbwd_ac[ng] = @variable(model, [1:nbranches_ac[ng]])
        plfwd_ac[ng] = @variable(model, [1:nbranches_ac[ng]])
        qlfwd_ac[ng] = @variable(model, [1:nbranches_ac[ng]])
        plbwd_ac[ng] = @variable(model, [1:nbranches_ac[ng]])
        qlbwd_ac[ng] = @variable(model, [1:nbranches_ac[ng]])
    end
    ...
    for ng = 1:ngrids
        ...
        # @constraint(model, pij_ac[ng] == vn_product[ng] .* (GG_ac[ng] .* cos.(theta_diff[ng]) .+ BB_ac[ng] .* sin.(theta_diff[ng])))
        # @constraint(model, qij_ac[ng] == vn_product[ng] .* (GG_ac[ng] .* sin.(theta_diff[ng]) .- BB_ac[ng] .* cos.(theta_diff[ng])))
        ...
        # Branch power balance constraint
        theta_diff_ft[ng] = theta_diff[ng][CartesianIndex.(fbus_ac[ng], tbus_ac[ng])]  
        theta_diff_tf[ng] = theta_diff[ng][CartesianIndex.(tbus_ac[ng], fbus_ac[ng])]
        @constraint(model, plfwd_ac[ng] == ((vm_ac[ng][fbus_ac[ng]].^2) .- vm_ac[ng][fbus_ac[ng]] .* vm_ac[ng][tbus_ac[ng]] .* cos.(theta_diff_ft[ng])) .* (-GG_ft_ac[ng]) .- vm_ac[ng][fbus_ac[ng]] .* vm_ac[ng][tbus_ac[ng]] .* sin.(theta_diff_ft[ng]) .* (-BB_ft_ac[ng]))
        @constraint(model, qlfwd_ac[ng] == -((vm_ac[ng][fbus_ac[ng]].^2) .- vm_ac[ng][fbus_ac[ng]] .* vm_ac[ng][tbus_ac[ng]] .* cos.(theta_diff_ft[ng])) .* (-BB_ft_ac[ng]) .- vm_ac[ng][fbus_ac[ng]] .* vm_ac[ng][tbus_ac[ng]] .* sin.(theta_diff_ft[ng]) .* (-GG_ft_ac[ng]))
        @constraint(model, plbwd_ac[ng] == ((vm_ac[ng][tbus_ac[ng]].^2) .- vm_ac[ng][tbus_ac[ng]] .* vm_ac[ng][fbus_ac[ng]] .* cos.(theta_diff_tf[ng])) .* (-GG_tf_ac[ng]) .- vm_ac[ng][tbus_ac[ng]] .* vm_ac[ng][fbus_ac[ng]] .* sin.(theta_diff_tf[ng]) .* (-BB_tf_ac[ng]))
        @constraint(model, qlbwd_ac[ng] == -((vm_ac[ng][tbus_ac[ng]].^2) .- vm_ac[ng][tbus_ac[ng]] .* vm_ac[ng][fbus_ac[ng]] .* cos.(theta_diff_tf[ng])) .* (-BB_tf_ac[ng]) .- vm_ac[ng][tbus_ac[ng]] .* vm_ac[ng][fbus_ac[ng]] .* sin.(theta_diff_tf[ng]) .* (-GG_tf_ac[ng]))
        @constraint(model, sl2fwd_ac[ng] == plfwd_ac[ng].^2 .+ qlfwd_ac[ng].^2)
        @constraint(model, sl2bwd_ac[ng] == plbwd_ac[ng].^2 .+ qlbwd_ac[ng].^2)
    end
...
for i in 1:nbuses_dc
    ...
    formatted_ps = @sprintf("%.3f", value(ps[i]) * baseMW_dc)
    formatted_qs = @sprintf("%.3f", value(qs[i]) * baseMW_dc)
    formatted_convPloss = @sprintf("%.3f", value(convPloss[i]) * baseMW_dc)
    ...
    println("-----  ----  ----  ---------    --------   -------  --------    --------")
    formatted_total_loss = @sprintf("%.3f", value(sum(convPloss)) * baseMW_dc)
