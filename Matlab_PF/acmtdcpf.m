...
[filepath,name,ext] = fileparts(mtdcFileName);
  ...
  [y_dc, ~, ~] = makeYbus(baseMW, bus_dc, branch_dc);
   y_dc = abs(full(y_dc));
  ...
  convState = zeros(nconvs, 1); 
   ...
   closs = closs./(basekVac.^2/baseMW);
   aloss = conv(:,19)/baseMW;
...
for i = 1:nconvs   
    ...
    if conv(i,4) == 1   % p control
        con_dc = [con_dc; ps(i) == -conv(i,6)/baseMW];
    elseif conv(i,4) == 2 % dc v control
            con_dc = [con_dc; vn_dc(i) == conv(i,8)];
    else % droop control 
        con_dc = [con_dc; pn_dc(i) == (conv(i,24)-1/conv(i,23)*(vn_dc(i)-conv(i,25)))/baseMW*(-1)];
    end
   % ac side control mode
    if conv(i,5) == 1   % q control
        con_dc = [con_dc; qs(i) == -conv(i,7)/baseMW];
    else % ac v control
...
%%  " Defining AC Grid Primal Variables " 
 ...
 theta_diff_ft = cell(ngrids, 1);
 theta_diff_tf = cell(ngrids, 1);
 GG_ft = cell(ngrids, 1);
 BB_ft = cell(ngrids, 1);
 GG_tf = cell(ngrids, 1);
 BB_tf = cell(ngrids, 1);
 ...
 plfwd_ac = cell(ngrids, 1);
 qlfwd_ac = cell(ngrids, 1);
 sl2fwd_ac = cell(ngrids, 1);
 plbwd_ac = cell(ngrids, 1);
 qlbwd_ac = cell(ngrids, 1);
 sl2bwd_ac = cell(ngrids, 1);
...
[YY{ng}, ~, ~] = makeYbus(baseMVA, bus_ac{ng}, branch_ac{ng});
GG{ng} = real(full(YY{ng})); 
BB{ng} = imag(full(YY{ng}));

pd{ng} = bus_ac{ng}(:,3)/baseMVA;
qd{ng} = bus_ac{ng}(:,4)/baseMVA; 

%%  " Defining AC Grid Primal Variables and Boundaries "
lb_ac = -inf(4*nbuses_ac+2*ngens+2*nbranches_ac, 1);
ub_ac = inf(4*nbuses_ac+2*ngens+2*nbranches_ac, 1);
...
% 7
k = 4*nbuses_ac+2*ngens;
lb_ac(k+(1:nbranches_ac)) = 0; % lb of transmission line
% ub_ac(k+(1:nbranches_ac)) = (branch_ac{ng}(:,6)/baseMVA).^2; % ub of transmission line
% ub_sub = ub_ac(k+(1:nbranches_ac));
% ub_sub(ub_sub == 0) = inf;
% ub_ac(k+(1:nbranches_ac)) = ub_sub;
ub_ac(k+(1:nbranches_ac)) = inf;

% 8
k = 4*nbuses_ac+2*ngens+nbranches_ac;
lb_ac(k+(1:nbranches_ac)) = 0; % lb of transmission line
% ub_ac(k+(1:nbranches_ac)) = (branch_ac{ng}(:,6)/baseMVA).^2; % ub of transmission line
% ub_sub = ub_ac(k+(1:nbranches_ac));
% ub_sub(ub_sub == 0) = inf;
...
var_ac{ng} = sdpvar(4*nbuses_ac+2*ngens+2*nbranches_ac, 1); 
...
sl2fwd_ac{ng} = var_ac{ng}(4*nbuses_ac+2*ngens+(1:nbranches_ac), 1);
sl2bwd_ac{ng} = var_ac{ng}(4*nbuses_ac+2*ngens+nbranches_ac+(1:nbranches_ac), 1);

plfwd_ac{ng} = sdpvar(nbranches_ac, 1);
qlfwd_ac{ng} = sdpvar(nbranches_ac, 1);
plbwd_ac{ng} = sdpvar(nbranches_ac, 1);
qlbwd_ac{ng} = sdpvar(nbranches_ac, 1);

theta_ac_mat{ng} = repmat(va_ac{ng}, 1, nbuses_ac);
theta_ac_mat_t{ng} = repmat(va_ac{ng}.', nbuses_ac, 1);
theta_ac_diff{ng} = theta_ac_mat{ng} - theta_ac_mat_t{ng}; 
vn_ac_product{ng} = vm_ac{ng}*vm_ac{ng}.';
theta_diff_ft{ng} = theta_ac_diff{ng}(sub2ind(size(theta_ac_diff{ng}), fbus, tbus));
theta_diff_tf{ng} = theta_ac_diff{ng}(sub2ind(size(theta_ac_diff{ng}), tbus, fbus));
GG_ft{ng} = GG{ng}(sub2ind(size(GG{ng}), fbus, tbus));
BB_ft{ng} = BB{ng}(sub2ind(size(BB{ng}), fbus, tbus));
GG_tf{ng} = GG{ng}(sub2ind(size(GG{ng}), tbus, fbus));
BB_tf{ng} = BB{ng}(sub2ind(size(BB{ng}), tbus, fbus));
...
pij_ac{ng} = vn_ac_product{ng} .* (GG{ng} .* cos(theta_ac_diff{ng}) + BB{ng} .* sin(theta_ac_diff{ng}));
qij_ac{ng} = vn_ac_product{ng} .* (GG{ng} .* sin(theta_ac_diff{ng}) - BB{ng} .* cos(theta_ac_diff{ng}));
con_acdc{ng} = [con_acdc{ng}; pn_ac{ng} == sum(pij_ac{ng}, 2)];
con_acdc{ng} = [con_acdc{ng}; qn_ac{ng} == sum(qij_ac{ng}, 2)];
con_acdc{ng} = [con_acdc{ng}; plfwd_ac{ng} == ((vm_ac{ng}(fbus).^2) - vm_ac{ng}(fbus).*vm_ac{ng}(tbus).*cos(theta_diff_ft{ng})).*(-GG_ft{ng})-vm_ac{ng}(fbus).*vm_ac{ng}(tbus).*sin(theta_diff_ft{ng}).*(-BB_ft{ng})];
con_acdc{ng} = [con_acdc{ng}; qlfwd_ac{ng} == -((vm_ac{ng}(fbus).^2) - vm_ac{ng}(fbus).*vm_ac{ng}(tbus).*cos(theta_diff_ft{ng})).*(-BB_ft{ng})-vm_ac{ng}(fbus).*vm_ac{ng}(tbus).*sin(theta_diff_ft{ng}).*(-GG_ft{ng})];
con_acdc{ng} = [con_acdc{ng}; plbwd_ac{ng} == ((vm_ac{ng}(tbus).^2) - vm_ac{ng}(tbus).*vm_ac{ng}(fbus).*cos(theta_diff_tf{ng})).*(-GG_tf{ng})-vm_ac{ng}(tbus).*vm_ac{ng}(fbus).*sin(theta_diff_tf{ng}).*(-BB_tf{ng})];
con_acdc{ng} = [con_acdc{ng}; qlbwd_ac{ng} == -((vm_ac{ng}(tbus).^2) - vm_ac{ng}(tbus).*vm_ac{ng}(fbus).*cos(theta_diff_tf{ng})).*(-BB_tf{ng})-vm_ac{ng}(tbus).*vm_ac{ng}(fbus).*sin(theta_diff_tf{ng}).*(-GG_tf{ng})];
con_acdc{ng} = [con_acdc{ng}; sl2fwd_ac{ng} == plfwd_ac{ng}.^2+qlfwd_ac{ng}.^2];
con_acdc{ng} = [con_acdc{ng}; sl2bwd_ac{ng} == plbwd_ac{ng}.^2+qlbwd_ac{ng}.^2];
...
for i = 1:nbuses_dc
        fprintf(fid,'\n %3d %5d %5d %9.3f %12.3f %9.3f %9.3f %11.3f', i, conv(i,2), conv(i,3), value(vn_dc(i)), value(pn_dc(i))*baseMW ...
            , value(ps(i))*baseMW, value(qs(i))*baseMW, value(convPloss(i)*baseMW));
end
fprintf(fid, '\n-----  ----  ----  ---------    --------   -------  --------    --------');
 fprintf(fid,'\n The total converter losses is %.3f MW',sum(value(convPloss))*baseMW);
 fprintf(fid,'\n');
