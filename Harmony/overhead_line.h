...
public:
	...
	Symbol organization; // Symbol for organization
	Conductors() : organization("", 0.0) {} // Default constructor for organization
};
...
public:
	...
	std::vector<std::vector<Basic>> P; // Example initialization
	std::vector<std::vector<Basic>> Z; // Example initialization
	bool eliminate = true;
...
//	void overhead_line(overhead_Line& tl, std::unordered_map<std::string, std::variant<int, double, std::tuple<std::vector<double>, std::vector<double>>, Symbol>>& kwargs);
	...
	void setZ(const std::vector<std::vector<Basic>>& newZ) {
		Z = newZ;
	}
};
